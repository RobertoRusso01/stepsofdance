...
fetchClientsButton.addEventListener("click", async () => {
  const response = await fetch("http://localhost:3000/api/clienti/");
...
searchButton.addEventListener("click", async () => {
  ...
  const response = await fetch(
    `http://localhost:3000/api/clienti/search?${queryString}`
  );
...
searchButtonSchool.addEventListener("click", async () => {
  ...
  // Fai la chiamata API con la query string generata
  const response = await fetch(
    `http://localhost:3000/api/clienti/search?${queryParams.join("&")}`
  );
